<x-slot name="header">
    <h2 class="text-center">Data Tanaman</h2>
</x-slot>
<div class="py-12">
    <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
        <div class="bg-white overflow-hidden shadow-xl sm:rounded-lg px-4 py-4">
            ...
            @if($isModalOpen)
            @include('livewire.tanaman-create')
            @endif
            <table class="table-fixed w-full">
                <thead>
                    <tr class="bg-gray-100">
                        ...
                        <th class="px-4 py-2">Deskripsi Tanaman</th>
                        <th class="px-4 py-2">Action</th>
                    </tr>
                </thead>
                <tbody>
                    @foreach($tanaman as $post)
                    <tr>
                        <td class="border px-4 py-2">{{ $post->id }}</td>
                        <td class="border px-4 py-2">{{ $post->nama_tanaman }}</td>
                        <td class="border px-4 py-2">{{ $post->deskripsi_tanaman}}</td>
